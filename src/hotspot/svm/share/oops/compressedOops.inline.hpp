...
inline oop CompressedOops::decode_not_null(narrowOop v) {
  ...
  // NOTE (chaeubl): method is used during heap initialization, so heap can still be null.
  assert(SVM_ONLY(Universe::heap() == nullptr ||) Universe::is_in_heap(result), "object not in heap " PTR_FORMAT, p2i(result));
  return result;
}
...
#ifdef SVM
  // This method is used for oops that are on the stack. In that case, null may be represented as the heap base.
  if ((address)v == base()) {
    return nullptr;
  }
#endif // !SVM
