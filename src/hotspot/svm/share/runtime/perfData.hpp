...
// Wrapper classes to simplify the access to the performance data objects that live in the image heap.
// Again, be careful not to include any virtual functions in those classes as we directly cast the image
// heap oops to pointers.

class G1TLABPerfData {
 public:
  PerfLongVariable* alloc_threads();
  PerfLongVariable* fills();
  PerfLongVariable* max_fills();
  PerfLongVariable* alloc();
  PerfLongVariable* gc_waste();
  PerfLongVariable* max_gc_waste();
  PerfLongVariable* refill_waste();
  PerfLongVariable* max_refill_waste();
  PerfLongVariable* slow_alloc();
  PerfLongVariable* max_slow_alloc();
};

class G1GCPolicyPerfData {
 public:
  PerfLongVariable* desired_survivor_size();
  PerfLongVariable* gc_time_limit_exceeded();
  PerfLongVariable* max_tenuring_threshold();
  PerfLongVariable* tenuring_threshold();
};

class G1GCCausesPerfData {
 public:
  PerfStringVariable* cause();
  PerfStringVariable* last_cause();
};

class G1CollectorPerfData {
 public:
  PerfLongCounter* invocations();
  PerfLongVariable* last_entry_time();
  PerfLongVariable* last_exit_time();
  PerfLongCounter* time();
};

class G1SpacePerfData {
 public:
  PerfLongVariable* capacity();
  PerfLongVariable* init_capacity();
  PerfLongVariable* max_capacity();
  PerfLongVariable* used();
};

class G1GenerationPerfData {
 public:
  PerfLongVariable* capacity();
  PerfLongVariable* max_capacity();
  PerfLongVariable* min_capacity();
  G1SpacePerfData* space(int ordinal);
};

class G1AgeTablePerfData {
 public:
  PerfLongVariable* entry(int age);
};

class G1CpuTimePerfData {
 public:
  PerfLongCounter* gc_total();
  PerfLongCounter* gc_parallel_workers();
  PerfLongCounter* gc_conc_mark();
  PerfLongCounter* gc_conc_refine();
  PerfLongCounter* gc_service();
};

class G1PerfData {
 public:
  static G1PerfData* get() { return (G1PerfData*)SVMIsolateData::_performance_data; };

  G1TLABPerfData* tlab();
  G1GCPolicyPerfData* gc_policy();
  G1GCCausesPerfData* causes();
  G1CollectorPerfData* collector(int ordinal);
  G1GenerationPerfData* generation(int ordinal);
